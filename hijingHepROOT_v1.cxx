...
{
	if (argc < 3) {	
		std::cout << "Usage: " << argv[0] << " HepMC_output.hepmc" << " ROOT_output.root" <<std::endl;
		std::cout << "To specify config file: " << argv[0] << " HepMC_output.hepmc ROOT_output.root config_file_name" << std::endl; 
		return 1;
	}

	std::string hepmc_outfile = argv[1];
	std::string root_outfile = argv[2];

	if (argc == 4) { // if config file was specified
	...
	int intpar[8]; // array to hold int parameters from config file
	float flopar[3]; // array to hold float parameters from config file

	for (int i = 0; i < 8; i++) { // first 8 parameters in config file are ints
		std::getline(infile, line, ' ');	
		intpar[i] = std::stoi(line.c_str());	
		std::getline(infile, line); // skip to end of line
	}

	for (int i = 0; i < 3; i++) { // last 3 parameters in config file are floats
		std::getline(infile, line, ' ');
	...
	// The input parameters need to be of length 9 because HIJING is weird and Fortran77 is evil
	char FRAME[2][9]; 
	...
	// Output 
    std::string hepmc_outdir = outdir + hepmc_outfile;
    std::string root_outdir = outdir + root_outfile;
    std::ofstream outputStream(hepmc_outdir); // hepmc
    WriterAscii outputWriter(outputStream); // hepmc
    TFile f(root_outdir.c_str(), "recreate"); // root
	...
	// Done initializing ROOT stuff =========================================

	// Nuclear radius (formula from liquid drop model) (this stuff needed for HepMC)
	...
	hiparnt.ihpr2[20] = 1; // option to keep information of all particles including those which have decayed (HIJING bug, in Zoltan's hands)
	...
	for (int IE = 0; IE < N_EVENT; IE++) { // loop over events
		...
		if (himain1.natt <= 1) continue; // if number of particles <=1
		...
		if (FRAMEID==0){ // collider mode
			...
			BeamPT = std::make_shared<GenParticle>(FourVector(0.0, 0.0, 0.0, mass_au_gev), IonBeamT, 4); 
		}
		...
		//  Think of eccentricity like a Fourier expansion that we calculate out to 10th order
		// ---------------------------------------------------------------------------------
		...
		for (int part=0; part < himain1.natt; part++){ // Loop over hijing particles
			// HepMC stuff
			int mid = himain2.katt[2][part]-1; // index of parent particle
		...
		// TODO: combine this next loop with filling hijTree branches? Check that they're the same
		for (unsigned int ipart=0; ipart<Hep_particles.size(); ipart++){ // loop over hepmc particles
			...
			if (mid < 0){ // idk how line number of parent particle could be < 0... maybe these are nucleons that underwent elastic scattering?
				...
				Prod_vertex = Hep_prods[mid]; // get parent particle vertex 
